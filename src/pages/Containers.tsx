...
const Containers = () => {
  ...
  const handleTakeOrder = useCallback(async (order: OrderWithDetails) => {
    ...
    try {
      ...
      // Get the logged-in user UUID from auth context
      const userId = user.id;

      // Fetch transporter bigint ID from a profile or mapping table
      // Replace 'profiles' and column names if your schema differs
      ...
      if (profileError || !profile) {
        toast.error('Failed to get transporter ID');
        console.error('Profile error:', profileError);
        return;
      }

      const transporterId = profile.transporter_id; // bigint transporter ID

      // Fetch the vehicle linked to transporter bigint ID
      ...
      if (vehicleError || !vehicle) {
        toast.error('Failed to get vehicle');
...
// Update the container assignment
    const { error: containerError } = await supabase
      .from('containers')
      .update({
        assigned_to: userId,  // UUID is okay here
    ...
    toast.success('Order taken and container assigned!');
  } catch (err: any) {
    ...
    toast.error(errorMessage);
      ...
      const startLocationTracking = useCallback(() => {
        ...
        const trackingInterval = setInterval(async () => {
          try {
            ...
            setState(prev => ({
              ...
              routeCoordinates: [...(prev.routeCoordinates || []), newLocation]
            }));
          } catch (error) {
        ...
        updateState({ trackingIntervalId: trackingInterval });
      }, [updateState, getCurrentLocation]);

      const stopLocationTracking = useCallback(() => {
        setState((prev: ContainersState) => {
          // Clear the tracking interval if it exists
          if (prev.trackingIntervalId) {
            clearInterval(prev.trackingIntervalId);
          }
          return {
            ...prev,
            isTracking: false,
          ...
          <div className="p-6 bg-gray-50 min-h-screen">
            ...
            {state.mapVisible && state.currentOrder && (
              <TrackingMap
                ...
                routeCoordinates={state.routeCoordinates.map(coord => [coord.lat, coord.lng])}
